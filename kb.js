      ...
      return name || "keyboard-layout";
		...
		$scope.moveKeys = function(x,y,$event) {
			...
			transaction("move", function() {
				$scope.selectedKeys.forEach(function(selectedKey) {
					selectedKey.x = Math.max(0,selectedKey.x + x),-2;
					selectedKey.y = Math.max(0,selectedKey.y + y),-2;
					renderKey(selectedKey);
				});
				$scope.multi = angular.copy($scope.selectedKeys.last());
			});
		...
		$scope.sizeKeys = function(x,y,$event) {
			...
			transaction("size", function() {
				$scope.selectedKeys.forEach(function(selectedKey) {
					update(selectedKey, 'width', validate(selectedKey, 'width', Math.max(1,selectedKey.width + x),-2));
					update(selectedKey, 'height', validate(selectedKey, 'height', Math.max(1,selectedKey.height + y),-2));
					renderKey(selectedKey);
				});
				$scope.multi = angular.copy($scope.selectedKeys.last());
			});
		...
		$scope.moveCenterKeys = function(x,y,$event) {
			...
			transaction("moveCenter", function() {
				$scope.selectedKeys.forEach(function(selectedKey) {
					update(selectedKey, 'rotation_x', validate(selectedKey, 'rotation_x', $scope.multi.rotation_x + x,-2));
					update(selectedKey, 'rotation_y', validate(selectedKey, 'rotation_y', $scope.multi.rotation_y + y,-2));
					renderKey(selectedKey);
				});
				$scope.multi = angular.copy($scope.selectedKeys.last());
			});
